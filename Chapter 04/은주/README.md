...
- final 키워드는 클래스, 메서드, 필드 또는 참조에 사용될 수 있다
  ...
  - final 변수 참조 : 필드처럼 동작하며 재할당 불가. **참조형 변수에 들어있는 참조값만 변경하지 못한다는 뜻이고 참조하는 대상의 값은 변경 가능**
    ...
    ```
- 불변성 부족으로 인해 발생하는 대부분의 문제는 기저 자료구조 자체에서 나오며 재할당된 참조와는 관련이 없다
- 사용중인 동안 자료 구조가 **예상치 못하게 변경되지 않도록 보장하려면 처음부터 불변 자료 구조를 선택해야 한다**
  - 이 목표를 달성하기 위한 자바 최신 추가 기능은 레코드 클래스이다.

### 4.3.8. 레코드
- 레코드는 주로 상태 선언으로 구성된 얕은 불변성을 가진 데이터 운반체이다
- 자동으로 생성된 getter, 동등성 비교, toString 및 hashCode 메서드 등을 제공한다

## 4.4. 불변성 만들기
- 타입을 불변하게 만드는 가장 쉬운 방법은 처음부터 데이터를 수정할 수 없도록 설계하는 것이다.
  - 하지만 실제 코드에서는 이 방법을 항상 적용할 수 있는 건 아니다
- 공유 자료 구조는 보통 한 번에 전부 생성되지 않는다.
  - 즉, 시간의 흐름에 따라 자료 구조를 변경하는 대신 마지막에는 'final' 불변 자료 구조를 완성한다
- **개별 컴포넌트들이 불변하지 않을 수 있지만, '최종적으로' 는 항상 불변**한 셸 (record 혹은 다른 방식) 로 감싸려고 노력해야 한다