...
### 4.4.1. 일반적인 관행
- 함수형 접근 방식처럼 불변성도 항상 절대적이지 않아도 된다

#### 기본적인 불변성
- 새로운 자료 구조는 불변하도록 설계되어야 한다
- 라이브러리나 프레임워크에서 **불변성을 가진 대안을 제공한다면, 가변 타입보다 그것을 우선적으로 고려해야 한다**

#### 항상 불변성을 고려하기
- 컬렉션과 같은 타입을 다룰 때, **하나를 변경해야 한다면 기존의 것을 기반으로 새로운 것을 만드는 것이 더 안전하다**

#### 기존 타입 수정하기
- 기존 타입이 불변하지 않더라도, **가능하면 새롭게 추가되는 사항들은 불변해야 한다**

#### 필요한 경우 불변성 깨기
- 나의 코드와 불변성이 맞지 않는다면, 강제로 바꾸려고 하지 마라
- 불변성의 주요 목표는 `안전하고 합리적인 자료 구조를 제공하는 것`이며, 이를 위해서는 **개발 환경이 적절히 뒷받침되어야 한다**

#### 외부 자료 구조 불변 처리하기
- 메서드 인수로 받은 컬렉션과 유사한 타입은 불변하다고 가정해야 한다.
  - 이 경우, 직접 조작하는 대신 변경을 위한 가변 래퍼 뷰를 만들고, 수정할 수 없는 컬렉션 타입을 반환해야 한다.
- 이 방법은 메서드를 순수하게 유지하고, **호출자가 예상하지 못한 의도치 않은 변경을 방지한다.**

## 핵심 요약 
- 레코드는 불변 자료 구조를 생성하기 위해 의도적으로 유연성을 포기하여, 가능한 명료하고 직관적인 방식을 제안한다
- 일반적인** 불변성의 원칙을 기존 코드에 점진적으로 적용하여, 상태 관련 버그를 줄이고 리팩터링 작업을 더 쉽게 할 수 있다**