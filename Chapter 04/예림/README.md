...
### 4.3.7 final 키워드
- final은 모든 상황에서 자료 구조를 불변하게 만드는 마법 같은 키워드는 아니다.
- 참조 자체는 불변이 되지만 기저 자료 구조는 불변하지 않는다.
- 사용 중인 동안 자료구조가 예상치 못하게 변경되지 않도록 보장하려면 **처음부터 불변 자료 구조를 선택**해야 한다. 이 목표를 달성하기 위한 자바의 최신 기능은 레코드이다.

### 4.3.8 레코드
- 자바 14에서 POJO와 JavaBean을 보완하고, 특정 상황에 대체할 수 있는 새로운 클래스 타입인 레코드를 도입했다.
- 레코드는 '기본 데이터'의 집합체이며, POJO나 JavaBean에 비해 간결하고 목표지향적이다.
```java
...
- 타입을 불변하게 만드는 가장 쉬운 방법은 처음부터 데이터를 수정할 수 없도록 설계하는 것
  - 설정자가 없는 경우에, final 필드를 가진 자료구조는 처음 생성되면 그 후로는 변경 불가능함
- 하지만 실제 코드에서는 이 방법을 항상 적용할 수 있는 것은 아님
- **데이터의 불변성**은 새로운 사고방식을 요구함
- 공유 자료 구조는 가능한 한 불변한 구조를 중간 단계에서 사용하고, 마지막에 final 불변 자료 구조를 완성함
- 개별 컴포넌트들이 불변하지 않을 수 있지만, 항상 불변한 셸로 감싸려고 노력해야 함.
